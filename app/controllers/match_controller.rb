class MatchController < ApplicationController
    ...
    def search_user
        if current_user
            if params[:interest]
                userids = PlayerInterestMapping.using(:games_db).where("interest = ?", "#{params[:interest]}").select("user_id")
                @users = User.using(:users_db).where('username LIKE ? OR id IN (?)',"%#{params[:interest]}%", userids)
            else
                @users = User.using(:users_db).all
            end
        end
    end 
