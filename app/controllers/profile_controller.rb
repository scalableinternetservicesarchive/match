class ProfileController < ApplicationController
  ...
  def show
    if current_user
      ...
      accepted_game_ids = PlayerGameMapping.where(user_id:current_user.id,is_organizer:false).select("game_id")
      @accepted_games = Game.where('id IN (?)',accepted_game_ids)
      created_game_ids = PlayerGameMapping.where(user_id:current_user.id,is_organizer:true).select("game_id")
